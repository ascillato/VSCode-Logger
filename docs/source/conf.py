...
def _generate_complexity_report() -> bool:
    ...
    try:
        env = os.environ.copy()
        env["COMPLEXITY_THRESHOLD_CYCLOMATIC"] = str(
            _complexity_thresholds["cyclomatic"]
        )
        env["COMPLEXITY_THRESHOLD_LOC"] = str(_complexity_thresholds["loc"])
        env["COMPLEXITY_THRESHOLD_DIT"] = str(_complexity_thresholds["dit"])
        env["COMPLEXITY_THRESHOLD_CBO"] = str(_complexity_thresholds["cbo"])
        result = subprocess.run(
