...
## [1.9.18] - UNRELEASED [CONFIG_IMPROVEMENTS]
...
### Removed
- Changes related to VSCode Refresh on v1.9.12.
- OpenSpec framework files.
...
- OpenSpec framework files for testing this new development framework.
...
### Changed
- Fixed highlight when hovering devices names in devices list.
...
- Added optional device ping monitoring (`embeddedLogger.enableDevicePing`) with an optional interval (`embeddedLogger.devicePingIntervalSeconds`), a Devices view Ping action, and live green/red reachability dots beside device names.

### Changed
- Improved wording in UI for passwords.
- Enabled for new installations the global defaults for SSH terminal, SFTP explorer, external web browser, embedded web browser, and device ping.
- Lighten device options background color.
...
- Added a per-device `showDefaultSshCommands` option and Device Manager column to control whether shared SSH commands are shown before device-specific commands in the Devices view.
