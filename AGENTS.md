...
* **Device tree and side‑panel views** – the activity bar lists devices defined under the `embeddedLogger.devices` setting. Selecting a device opens a log panel for that device. The side panel also lets users manage up to ten highlight keywords, run one‑off SSH commands, open interactive SSH terminals, and launch device URLs in an external or embedded browser.
...
* **Configuration file** – groups are configured via `embeddedLogger.groups` and devices via `embeddedLogger.devices` in `settings.json`. Each device entry specifies an `id`, optional `group`, optional `color`, `name`, `host`, optional `port`, `username`, optional `password` (used only to migrate into secret storage), an optional `privateKeyPath`, an optional `logCommand`, optional browser toggles `enableWebBrowser` / `enableEmbeddedWebBrowser`, an optional `webBrowserUrl`, optional `sftpPresetsRemote` / `sftpPresetsLocal`, and optional `sshCommands`. Additional top‑level settings such as `embeddedLogger.defaultPort`, `embeddedLogger.defaultLogCommand`, and `embeddedLogger.maxLinesPerTab` set sensible defaults when omitted. The Device Manager can export/import this full settings block as JSON, and import validates required keys before loading it into the table UI.
...
| `src/logPanel/`          | Hosts the Webview panel for a device and wires it to an SSH log session. `logPanel.ts` owns lifecycle and messaging, `html.ts` builds the CSP‑backed markup, `stateStore.ts` persists presets/highlights, `messageParser.ts` validates inbound messages and `autoSaveManager.ts` manages on‑disk streaming. An `index.ts` re‑exports the entrypoint. |
| `src/logSession/`        | Handles SSH connectivity and streaming via composable pieces: `authenticationProvider.ts` (secrets and key loading), `connectionManager.ts` (channel setup and stream lifecycles), `hostKeyVerifier.ts` / `fingerprintPersistence.ts` (verification and capture), `reconnectionController.ts` (retry orchestration) and `logSession.ts` (public façade). |
| `src/sshCommandRunner.ts` | Provides the `SshCommandRunner` class for executing one‑off SSH commands defined in the device configuration. Validates device configuration, sanitises commands to prevent injection, obtains authentication via `PasswordManager` and executes the command, returning stdout/stderr.                                                 |
| `src/sshTerminal.ts`     | Provides the `SshTerminalSession` pseudoterminal for interactive SSH shells. Handles input/output, window resize events and authentication via `PasswordManager`.                                                                                                                                                                   |
| `src/passwordManager.ts` | Provides helpers for storing and retrieving device passwords and passphrases using workspace‑aware keys. Supports migrating secrets from previous versions, reusing secrets across workspaces and cleaning up secrets.                                                                                                             |
...
| `src/sftpExplorer.ts`    | Owns the SFTP explorer Webview panel, directory listings, and SFTP operations for remote/local panes.                                                                                                                                                                                                                 |
| `media/loggerPanel/`     | Webview client for the log panel split into `state.js` (state persistence and restoration), `rendering.js` (DOM helpers and highlight painting) and `messaging.js` (postMessage handlers). `media/loggerPanel.js` is the module entrypoint that composes these pieces. Maintains entries, filters, presets, highlights, bookmarks and search. |
...
| `media/sidebarView.js`   | Client‑side JavaScript loaded in the side‑panel Webview. Renders device cards, highlight rows and command buttons. Enforces a maximum of ten highlight keywords, generates colour pairs, handles add/remove actions and posts messages back to the extension.                                      |
...
| `media/sftpExplorer.js`  | Webview client for the SFTP explorer. Handles rendering, selection, quick search, context menus, and keyboard shortcuts.                                                                                                                                                                                                |
| `media/sftpExplorer.css` | Styles the SFTP explorer Webview, including list layout, selection states, quick search labels, and dialogs.                                                                                                                                                                                                          |
...
The log session and log panel are intentionally modular: keep SSH orchestration inside `src/logSession/` helpers and UI wiring inside `src/logPanel/` helpers. On the frontend, continue to route state changes through `media/loggerPanel.js` and its `state`/`rendering`/`messaging` modules so highlights, presets and filters stay in sync.
...
4. **Configure devices**: in VS Code settings (or via `settings.json`), add entries under `embeddedLogger.devices` as shown in the README. Each entry can include `id`, optional `color`, `name`, `host`, `port`, `username`, `privateKeyPath`, `password` (only for migrating to secret storage), `logCommand`, `sftpPresetsRemote`, `sftpPresetsLocal`, and `sshCommands` (an array of `{name, command}`). Do not hard‑code devices in the source.
...
* **Do not store secrets in code**: always retrieve passwords and passphrases via `PasswordManager`. When adding new sensitive data (e.g., API tokens), use `context.secrets` and workspace‑aware keys. Never commit secrets to the repository.
...
* **Document your changes**: update `README.md`, `CONTRIBUTING.md` and this file `AGENTS.md` when adding features or altering workflows. Explain new configuration fields, UI elements and behaviours clearly.
...
* **SSH commands and terminals**: to support additional command types or terminal features, extend `src/sshCommandRunner.ts` and `src/sshTerminal.ts`. Sanitize commands to prevent injection, validate device configuration and handle authentication via `PasswordManager`. Provide clear status messages using `window.showInformationMessage` or Webview events.
