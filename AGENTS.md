...
* **Document your changes**: update `README.md`, `CONTRIBUTING.md` and `AGENTS.md` when adding features or altering workflows.  Explain new configuration fields, UI elements and behaviours clearly.
...
* **Bastion hosts**: devices may now include a `bastion` block with its own host, username, optional port, host fingerprint and authentication (password or private key).  All SSH entry points (logs, commands, terminal, SFTP) should tunnel through the bastion when configured and capture/store bastion fingerprints separately from device fingerprints.
