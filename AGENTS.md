...
* **Configuration file** – devices are configured in `settings.json` via `embeddedLogger.devices`. Each entry specifies an `id`, optional `color`, `name`, `host`, optional `port`, `username`, optional `password` (used only to migrate into secret storage), an optional `privateKeyPath`, an optional `logCommand`, optional browser toggles `enableWebBrowser` / `enableEmbeddedWebBrowser`, an optional `webBrowserUrl`, and optional `sshCommands`. Additional top‑level settings such as `embeddedLogger.defaultPort`, `embeddedLogger.defaultLogCommand`, and `embeddedLogger.maxLinesPerTab` set sensible defaults when omitted.
...
4. **Configure devices**: in VS Code settings (or via `settings.json`), add entries under `embeddedLogger.devices` as shown in the README. Each entry can include `id`, optional `color`, `name`, `host`, `port`, `username`, `privateKeyPath`, `password` (only for migrating to secret storage), `logCommand` and `sshCommands` (an array of `{name, command}`). Do not hard‑code devices in the source.
