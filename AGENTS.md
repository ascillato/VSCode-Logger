...
* **Configuration file** – devices are configured in `settings.json` via `embeddedLogger.devices`. Each entry specifies an `id`, optional `color`, `name`, `host`, optional `port`, `username`, optional `password` (used only to migrate into secret storage), an optional `privateKeyPath`, an optional `logCommand`, and optional `sshCommands`. Additional top‑level settings such as `embeddedLogger.defaultPort`, `embeddedLogger.defaultLogCommand`, and `embeddedLogger.maxLinesPerTab` set sensible defaults when omitted.
...
7. **Documentation**: run TypeDoc (`npm run docs:typedoc`) from the repository root. The generated HTML lives at `docs/typedoc/index.html`. Keep `README.md`, `CONTRIBUTING.md` and this file `AGENTS.md` up to date when introducing new behaviour.
