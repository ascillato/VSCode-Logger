...
* **Configuration file** – devices are configured in `settings.json` via `embeddedLogger.devices`. Each entry specifies an `id`, optional `color`, `name`, `host`, optional `port`, `username`, optional `password` (used only to migrate into secret storage), an optional `privateKeyPath`, an optional `logCommand`, optional browser toggles `enableWebBrowser` / `enableEmbeddedWebBrowser`, an optional `webBrowserUrl`, optional `sftpPresetsRemote` / `sftpPresetsLocal`, and optional `sshCommands`. Additional top‑level settings such as `embeddedLogger.defaultPort`, `embeddedLogger.defaultLogCommand`, and `embeddedLogger.maxLinesPerTab` set sensible defaults when omitted. The Device Manager can export/import this full settings block as JSON, and import validates required keys before loading it into the table UI.
