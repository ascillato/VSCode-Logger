...
export class SftpExplorerPanel {
  ...
  private async handleMessage(message: WebviewRequest): Promise<void> {
    try {
      switch (message.type) {
        ...
        case 'renameEntry': {
          const refreshDir = await this.renameEntry(
            message.location,
            message.path,
            message.newName
          );
        ...
        case 'createDirectory': {
          const refreshDir = await this.createDirectory(
            message.location,
            message.path,
            message.name
          );
  ...
  private async buildSnapshot(
    ...
    const normalizedPath = this.normalizePath(
      location,
      dirPath || (location === 'remote' ? (this.remoteHome ?? '/') : this.localHome)
    );
    await this.ensureDirectoryExists(location, normalizedPath);
    const entries =
      location === 'remote'
        ? await this.listRemote(normalizedPath)
        : await this.listLocal(normalizedPath);
    if (location === 'remote') {
      if (context === 'right') {
  ...
  private formatPermissions(mode: number | undefined): string {
    ...
    return symbols.map((symbol, index) => ((mode & flags[index]) !== 0 ? symbol : '-')).join('');
  }
  ...
  private async renameEntry(
    ...
    const destination =
      location === 'remote' ? path.posix.join(parent, newName) : path.join(parent, newName);
  ...
  private async duplicateEntry(location: 'remote' | 'local', targetPath: string): Promise<string> {
    ...
    const destination =
      location === 'remote'
        ? path.posix.join(parent, duplicateName)
        : path.join(parent, duplicateName);
  ...
  private async confirmOverwrite(
    ...
    const baseName =
      location === 'remote' ? path.posix.basename(destinationPath) : path.basename(destinationPath);
  ...
  private async getPermissionsInfo(
    ...
    const { ownerName, groupName } = await this.resolveOwnerGroupNames(
      location,
      stats.uid,
      stats.gid
    );
  ...
  private async generateCopyName(
    ...
    while (true) {
      ...
      const candidatePath =
        location === 'remote'
          ? path.posix.join(directory, candidate)
  ...
  private async copyRemoteDirectory(
    source: string,
    destination: string,
    mode?: number
  ): Promise<void> {
        ...
        .on('ready', () => {
          bastionClient.forwardOut(
            ...
            (err: Error | undefined, stream: ClientChannel) => {
              ...
              void this.connectDirect(endpoint, auth, expectedFingerprint, stream, () =>
                bastionClient.end()
              )
                .then(resolve)
                .catch((error: unknown) => {
      ...
      const passphrase = await this.passwordManager.getPassphrase(this.device, {
        onPrompt: () =>
          this.updateConnectionStatus(
            'reconnecting',
            undefined,
            'Waiting for the user to enter the password…'
          ),
      });
  ...
  private getHtml(webview: vscode.Webview): string {
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.file(path.join(this.context.extensionPath, 'media', 'sftpExplorer.js'))
    );
    const styleUri = webview.asWebviewUri(
      vscode.Uri.file(path.join(this.context.extensionPath, 'media', 'sftpExplorer.css'))
    );
