...
export class LogSession {
  ...
  private async persistFingerprintIfMissing(): Promise<void> {
    ...
    await this.updateDeviceHostFingerprint(
      this.lastSeenHostFingerprint.display,
      this.activeEndpoint
    );
  ...
  private createForwardingClient(): ForwardingClient {
    return (this.dependencies.createForwardingClient?.() ??
      (new Client() as ForwardingClient)) as ForwardingClient;
  }
}
